...
export function startWildEncounter() {
  ...
  const lvl = 3 + Math.floor(Math.random() * 5) + (map.encounter === 'cave' ? 3 : 0);
...
export function talkTo(npc) {
  const talks = {
    sage: [
      ...
      'Orme: Tu Emberkit te protegerá. ¡Ve con el guardabosques Vela!',
    ],
    shop: [
      'Nia: ¡Hola! Tengo orbes ámbar y pociones si los necesitas.',
      'Nia: Pulsa P para ver tu party y usar pociones.',
    ],
    brok: [
